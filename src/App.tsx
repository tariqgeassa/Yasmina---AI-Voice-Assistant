...
import { Mic, MicOff, Loader2, Volume2, VolumeX, Keyboard, Send, Trash2, Maximize, Minimize } from "lucide-react";
import { getYASMINAResponse, getYASMINAAudio, resetYASMINASession } from "./services/geminiService";
...
export default function App() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    const saved = localStorage.getItem("YASMINA_chat_history");
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("YASMINA_chat_history", JSON.stringify(messages));
  }, [messages]);
  ...
  const requestWakeLock = async () => {
    // Standard WakeLock is often blocked in AI Studio iframe
    // Fallback: Periodically tweak the DOM to hint activity
    if (keepAwakeIntervalRef.current) return;
  ...
  const handleTextCommand = useCallback(async (finalTranscript: string) => {
    ...
    if (commandResult.isBrowserAction) {
      ...
      setMessages((prev) => [...prev, { id: Date.now().toString() + "-y", sender: "YASMINA", text: responseText }]);
      
      if (!isMuted) {
        setAppState("speaking");
        const audioBase64 = await getYASMINAAudio(responseText);
        if (audioBase64) {
      ...
      responseText = await getYASMINAResponse(finalTranscript, messagesRef.current);
      setMessages((prev) => [...prev, { id: Date.now().toString() + "-y", sender: "YASMINA", text: responseText }]);
    ...
    <div className="h-[100dvh] w-screen bg-[#050505] text-white flex flex-col items-center justify-between font-sans relative overflow-hidden m-0 p-0">
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center z-20 shrink-0 px-6 py-4 md:px-12 md:py-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-serif font-medium tracking-wide opacity-90">YASMINA</h1>
        </div>
      ...
      <main className="absolute inset-0 flex flex-row items-center justify-between w-full h-full z-10 overflow-hidden pt-20 pb-24 px-4 md:px-12 pointer-events-none">
        ...
        {/* Chat History Overlay (Floating bubbles) */}
        <div className="absolute inset-x-0 top-24 bottom-32 px-6 md:px-20 overflow-y-auto scrollbar-hide pointer-events-auto flex flex-col gap-4">
          <AnimatePresence initial={false}>
            {messages.map((msg) => (
              <motion.div
                ...
                className={`max-w-[80%] md:max-w-[50%] p-4 rounded-2xl md:rounded-3xl backdrop-blur-md border ${
                  msg.sender === "YASMINA"
                    ? "self-start bg-violet-600/10 border-violet-500/20 text-violet-100 rounded-bl-none"
                    : "self-end bg-white/5 border-white/10 text-white rounded-br-none"
                }`}
              >
                <div className="text-[10px] uppercase tracking-widest opacity-40 mb-1 font-mono">
                  {msg.sender === "YASMINA" ? "YASMINA" : "User"}
                </div>
        ...
        <div className="hidden md:flex w-[30%] lg:w-[25%] h-full flex-col justify-center gap-4 z-10">
          ...
          {showTextInput && (
            ...
            >
              <input 
                ...
                placeholder="Type a message to YASMINA..."
